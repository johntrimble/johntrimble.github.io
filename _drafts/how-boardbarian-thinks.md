...
Rules questions are hard at every layer: finding the right rules, and then reasoning about them. As we'll see, that second layer is where small models struggle most.
...
Long contexts only make matters worse. Even though many small models can, on paper, handle large contexts, in practice Qwen3 30B A3B Instruct starts losing output quality once the context exceeds roughly 40k tokens. This means we must be especially economical with the context we provide to the model.
...
The rigid structure also bounds the damage a doom loop can do: the model is never free to wander off gathering context indefinitely, so a runaway loop costs at most one generation, not an unbounded agentic spiral. Loops within a single generation are a different matter: those are tamed mostly by tuning sampling parameters, which is a story about evaluations, and a topic for the next post in this series.
...
To deal with followup questions and ongoing conversations, the system maintains a conversation history with the user. However, the conversation history is always boiled down to a single query for the system to answer. This helps keep the context size small to improve reasoning performance, at the cost of a slower response time. This means the above workflows never see the full conversation history.
...
2. **Faster than the rulebook.** A simple question costs a single search and a single answering pass, and when a question does need decomposition, the subquestions are answered in parallel rather than one at a time. In practice, a question takes 20 to 50 seconds to answer; the Grail Knights question takes about **[TODO: Grail Knights timing]**. That comfortably beats digging out the rulebook, clearing space for it on the table, and finding the right passages.
...
4. **Auditable.** Every answer carries quotes that have been verified to actually appear in the rulebook, complete with page citations, so users can check the reasoning rather than take it on faith.
